...
```bash
python-helloWorld$ sam build --use-container
```
...
```bash
python-helloWorld$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
python-helloWorld$ sam local start-api
python-helloWorld$ curl http://localhost:3000/hello
```
...
```bash
python-helloWorld$ sam logs -n HelloWorldFunction --stack-name python-helloWorld --tail
```
...
```bash
python-helloWorld$ pip install -r tests/requirements.txt --user
# unit test
python-helloWorld$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
python-helloWorld$ AWS_SAM_STACK_NAME="python-helloworld" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "python-helloworld"
```
